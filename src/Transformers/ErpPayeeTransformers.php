        ...
        return [
            ...
            'payee_list' => $item->payee_list->map(function ($item) {
                return array_merge($item->toArray(), [
                    'warehousing_code'=>$item->payee_list_code,
                    'distribution_no'=>$item->payee_list_no,
                    'distribution_Transfer_order'=>$item->payee_list_transfer_order,
                    'customer_number'=>$item->payee_list_order,//订单号==入仓单号
                    'warehousing_money'=>$item->payee_list_amount,//应结金额
                    'csm_name'=>ErpCsm::where('csm_id',$item->agent_csm_id)->value('csm_name'),//结算方式
                    'distribution_insert_at'=>$item->bill_at,//单据时间
